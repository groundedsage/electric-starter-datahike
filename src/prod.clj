(ns prod
  (:gen-class)
  (:require [app.todo-list :as app] ; prod only (app is not loaded by shadow in prod) 
            clojure.string
...
(defn -main [& args] ; run with `clj -M -m prod`
  #_(when (clojure.string/blank? (System/getProperty "HYPERFIDDLE_ELECTRIC_SERVER_VERSION"))
